...
func NewMongoDBTagRepository(conn *database.MongoDBConn) TagRepository {
...
func (m *MongoDBTagRepository) Update(t *entity.Tags) error {
	coll := m.db.Collection(UserProductsCollection)

	res, err := coll.UpdateOne(*m.ctx, bson.M{"id": t.ProductId}, t)
